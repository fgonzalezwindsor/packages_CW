...
public class ModWindsorValidAvaiQtyOrder implements ModelValidator
{
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if((type == TYPE_BEFORE_NEW || type == TYPE_BEFORE_CHANGE)&& po.get_Table_ID()== MOrderLine.Table_ID)  
		{
								...
								/*	if(order.getC_BPartner_Location_ID() == 1010879 || order.getC_BPartner_Location_ID()  == 1011338)
									...
									else if(order.getM_Warehouse_ID() == 1000027)
									{
										...
										if(aux2 == null)
											aux2 = Env.ZERO;							
										if(aux2.subtract(oLine.getQtyOrdered()).compareTo(Env.ZERO) < 0)								
											return "ERROR: Stock Insuficiente: "+aux2.intValue()+". Stock en otras bodegas:"+qtyAvai.intValue()+" . <LC: 297>";
										else//si es la direccion pero no manda el error que reemplaze el disponible
										{
											//ininoles si la bodega es 1000027 no se valida nada mas que el stock.
											//qtyAvai = aux2;		
									...
									if(qtyAvai.subtract(oLine.getQtyOrdered()).compareTo(Env.ZERO) < 0)
										return "ERROR: Stock Insuficiente y NO Existen solicitudes abiertas. <LC: 305>";
			...
			{
				if(oLine.get_ValueAsInt("M_RequisitionLine_ID") > 0)
					return "ERROR. Linea de devolución NO debe tener solicitud asociada <LC: 380>";
			}
		}
							...
							&& oLine.getM_Product().getProductType().compareTo("I") == 0)
					{
						...
						else
						{
							...
							{							
								...
								if(order.get_ValueAsInt("OV_Prereserva_ID")>0)
								{
									...
									if(qtyAvai.subtract(oLine.getQtyOrdered()).compareTo(Env.ZERO) < 0)
										return "ERROR: Stock Insuficiente. <LC: 470>, Linea Nota: "+oLine.getLine() +", Producto: "+oLine.getM_Product().getName() + ", No Preventa: "+npr;
						...
						//		BigDecimal qtyAvai = Env.ZERO; 
								if(order.getAD_Client_ID() == 1000000)
								{
									...
									if(qtyAvai == null)
										qtyAvai = Env.ZERO;
									BigDecimal aux = DB.getSQLValueBD(po.get_TrxName(), 
											"SELECT "+ 
													//" qtyavailableofb(mp.M_Product_ID,1000010) FROM M_Product mp WHERE mp.M_Product_ID = "+oLine.getM_Product_ID());
													" COALESCE ( "+
												      "         (SELECT SUM (s.qtyonhand) "+
	                                ...
	                                }else if(order.getM_Warehouse_ID() == 1000027)
									{
										...
										if(aux2 == null)
											aux2 = Env.ZERO;							
										if(aux2.subtract(oLine.getQtyOrdered()).compareTo(Env.ZERO) < 0)								
											return "ERROR: Stock Insuficiente: "+aux2.intValue()+". Stock en otras bodegas:"+qtyAvai.intValue()+". <LC: 572>";
										else//si es la direccion pero no manda el error que reemplaze el disponible
										{
											//ininoles si la bodega es 1000027 no se valida nada mas que el stock.
											//qtyAvai = aux2;		
										...
										if(aux2 == null)
											aux2 = Env.ZERO;							
										if(aux2.subtract(oLine.getQtyOrdered()).compareTo(Env.ZERO) < 0)								
											return "ERROR: Stock Insuficiente: "+aux2.intValue()+". Stock en otras bodegas:"+qtyAvai.intValue()+" . <LC: 585>";
										else//si es la direccion pero no manda el error que reemplaze el disponible
										{
											//ininoles si la bodega es 1000027 no se valida nada mas que el stock.
											//qtyAvai = aux2;		
								...
								if(qtyAvai.subtract(oLine.getQtyOrdered()).compareTo(Env.ZERO) < 0)
									return "ERROR: Stock Insuficiente y NO Existen solicitudes abiertas. Linea OV:"+oLine.getLine() +". <LC: 616>";
