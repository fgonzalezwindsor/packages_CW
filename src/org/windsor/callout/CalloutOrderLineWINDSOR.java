...
public class CalloutOrderLineWINDSOR extends CalloutEngine
{
	...
	public String qtyDisponible (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		{		
			...
			String resultStr = "";
			Integer ware_id = (Integer) mTab.getValue("M_WareHouse_ID");
			...
			if (client_id==1000000)
			{
				if(product_id!=null && product_id!=0 )
				{
					...
					try {
						...
						if(ware_id>1000010)
						{
							String sql2="SELECT qtyavailableofb(?,?) as suma FROM dual";
							PreparedStatement pstmt2=null;
							try {
									pstmt2=DB.prepareStatement(sql2,null);
									pstmt2.setInt(1,product_id);
									pstmt.setInt(2,ware_id);
									ResultSet rs2= pstmt2.executeQuery();
									if(rs2.next())
									{
										disponible= rs2.getInt("suma");     
										resultStr=resultStr+" , y en la bodega de la nota: "+ disponible;
									}
								}
								catch (Exception e) {
