 ...
 */
public class ImportOrderB2CRF extends SvrProcess
{
	...
	try
	{
		...
		while (rs.next())
		{
			String sqlcreados = "Select count(o.DocumentNO) as cuenta from I_orderB2C o  where o.i_isImported<>'Y' "+
								" and o.C_BPartner_ID<>1001237 and o.documentno in (select b2.poreference from c_orderb2c b2 "+
	...
	int docnotok= 0;
		...
		for ( int i=0; i < ordenescompra.size() ; i++ ) // a recorrer Ordenes
		{
			String recorre = "Select documentno, COALESCE(c_bpartner_ID,0) C_BPartner_ID,  COALESCE(c_bpartner_location_ID,0)c_bpartner_location_ID,"+
							...
							" from  I_OrderB2C   where  C_BPartner_ID<>1001237 and documentNo='"+ ordenescompra.get(i)+"'";
				...
				while (rs.next()) //while crea ob2c 1 por 1
				{
					...
					if (ob2c==null)
					{
						//cabecera
						 ...
						 String mc ="";
						 if (!rs.getString ("tipogt").isEmpty() && rs.getString ("tipogt")!=null)
						 {
						 ...
						 String fc="";
						 if (!rs.getString ("formacgt").isEmpty() && rs.getString ("formacgt")!=null)
						 {
					...
					if (ob2c!=null)
					{
						...
						log.finer("M_product_ID="+rs.getInt("M_Product_ID"));
							...
							try
							{
								...
								if (rsp.next())
								{
									...
									String sqlqr =	"select count(1) encontrado "+
										...
										/*
										 * se modifica ultima linea por que se basara en qty reserved
										 */
									...
									try
									{
										...
										if(rsre.next())
										{
											log.fine("Codigo: "+prod.getValue() + "En Reserva Eccomerce: "+rsre.getInt("encontrado"));
											...
											{//si existe rf Ecommerce reabaja de esta,
												...
												try
												{
													...
													if(rsreq.next())
													{
														log.fine("Codigo: "+prod.getValue() + "En Reserva Eccomerce QtyR: "+rsreq.getInt("qtyreserved") + " Pedido: "+rs.getInt("Cantidad") );
														...
														{ //reserva eccomerce no cubre todo
															...
															BigDecimal neto = new BigDecimal (rsreq.getInt("qtyreserved") *rs.getInt("Precio") );
															obl.set_CustomColumn("M_RequisitionLine_ID",rsreq.getInt("M_RequisitionLine_ID") );
															obl.setLineNetAmt(neto);
															obl.setPASARAOV(true);
													...
													log.fine("Salto: "+salto + " Codigo: "+prod.getValue());
									...
									{//salto 0 o 2
										...
										//		
										log.info("Producto_ID:"+rs.getInt("m_product_ID"));
										log.info("BPartner_ID:"+rs.getInt("C_BPartner_ID"));
												String sqlrfn=" select count(1) encontrado "+ 
												 " from m_requisition r  "+
													...
													try
													{
														...
														if(rsrfn.next())
														{
															log.fine("Codigo: "+prod.getValue() + "En Reserva Fisica: "+rsrfn.getInt("encontrado"));
															//si existe rfisica,
															if(rsrfn.getInt("encontrado")>0)
															{
																...
																try
																{
																	...
																	{//reserva fisica normal rebajar
																		log.fine("Codigo: "+prod.getValue() + "En Reserva Eccomerce QtyR: "+rsrn.getInt("qtyreserved") + " Pedido: "+rs.getInt("Cantidad") );
																		...
																		{//cantiadad NO es cubierta por rf normal
																			...
																			obl.setQtyEntered(aux);
																			cant = new BigDecimal(cant.intValue() - aux.intValue());
																			BigDecimal neto = new BigDecimal (rsrn.getInt("qtyreserved") *rs.getInt("Precio") );
																			obl.set_CustomColumn("M_RequisitionLine_ID",rsrn.getInt("M_RequisitionLine_ID") );
																			obl.setLineNetAmt(neto);
																			obl.setPASARAOV(true);
																	...
																	log.finer("Salto: "+ salto + " Codigo: "+prod.getValue() );
									...
									try
									{
										...
										if(rsps.next())
										{
											log.fine("Codigo: "+prod.getValue() + "Didpinible : "+rsps.getInt("Disponible") + " Pedido: "+rs.getInt("Cantidad") );
